...
RUN mkdir -p /doctothon.io/hugo/src/
COPY . /doctothon.io/hugo/src/
COPY .git /doctothon.io/hugo/src/
RUN ls -allh /doctothon.io/hugo/src/
RUN export PATH=$PATH:/usr/local/go/bin && cd /doctothon.io/hugo/src/ && hugo -b "${HUGO_BASE_URL}"
...
RUN mkdir -p /doctothon.io/retrieved_hugo_build
COPY --from=hugo_build /doctothon.io/hugo/src/public /doctothon.io/retrieved_hugo_build
RUN echo "Right after retrieveing the result of the hugo build , content of  [/doctothon.io/retrieved_hugo_build] : "
RUN ls -allh /doctothon.io/retrieved_hugo_build
#   fRench do it, Let(')s do it, Let's...
RUN cp -fR /doctothon.io/retrieved_hugo_build/* /usr/local/apache2/htdocs
...
COPY heroku.apache.start.sh /doctothon.io
COPY heroku.container.healthcheck.sh /doctothon.io
RUN chmod +x /doctothon.io/*.sh

#
# healthcheck:
#   test: ["CMD", "/doctothon.io/website.healthcheck.sh"]
#   interval: 5s
#   timeout: 10s
#   retries: 30
#   start_period: 60s
...
ENTRYPOINT ["/doctothon.io/heroku.apache.start.sh"]
